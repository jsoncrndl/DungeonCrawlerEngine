...
namespace Engine::Graphics
{
	...
	{
		for (auto& function : m_eventCallbacks[static_cast<size_t>(event)])
		{
			function(data);
		}
	}

	void GameWindow::registerWindowEventListener(WindowEvent event, EventCallback callback)
	{
		if (m_eventCallbacks.size() == 0)
		{
			m_eventCallbacks[static_cast<size_t>(event)] = CallbackList(m_allocator);
		}

		auto& callbackList = m_eventCallbacks[static_cast<size_t>(event)];

		if (callbackList.size() >= MAX_CALLBACKS)
		{
			throw std::exception("Too many callbacks registered! Max allowed is " + MAX_CALLBACKS);
		}

		callbackList.push_back(callback);
	}

	GameWindow::GameWindow(Memory::PermanentAllocator allocator) : m_allocator(allocator)
	{
		...
		registerWindowEventListener(WindowEvent::RESIZE, [this](const WindowEventData& e) {
