...
namespace Engine::Graphics
{
	...
	void Graphics::freeShaders(std::shared_ptr<Resources::Registry<Shader, Resources::ShaderImporter>> registry)
	{
		for (auto& shader : registry->entries())
		{
	...
	void Graphics::freeTextures(std::shared_ptr<Resources::Registry<Texture, Resources::TextureImporter>> registry)
	{
		for (auto& shader : registry->entries())
		{
			glDeleteTextures(1, &shader.second->m_texture);
		}
	}
	...
	{
		auto assetManager = Engine::RuntimeEngine::Engine.getAssetManager();
		...
		m_window(window)
	{
		...
		//m_window->registerWindowEventListener(WindowEvent::RESIZE, [this](const WindowEventData& e) {
		//	updateViewport();
		//	}
		//);
